...
export const getAllComments = (productId) => async (dispatch) => {
  ...
  try {
    const res = await axios.get(
      `http://localhost:4002/api/product/comments/${productId}/allcomments`,
      config
    );
...
export const addComment = (productId, text) => async (dispatch) => {
  ...
  try {
    const res = await axios.post(
      `http://localhost:4002/api/product/comments/${productId}`,
      { text },
      config
    );
    console.log(res.data);
...
export const deleteComment = (productId, commentId) => async (dispatch) => {
  ...
  try {
    await axios.delete(
      `http://localhost:4002/api/product/comments/${productId}/deletecomment/${commentId}`,
