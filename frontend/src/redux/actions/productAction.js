...
export const getAllProducts = () => async (dispatch) => {
  ...
  try {
    const res = await axios.get(
      "http://localhost:4002/api/products/allproducts",
      config
    );
    console.log(res);
    dispatch({
...
export const addProduct = (formData, navigate) => async (dispatch) => {
  ...
  try {
    await axios.post(
      "http://localhost:4002/api/products/addproduct",
      formData,
      config
    );
...
export const editProduct = (productId, formData) => async (dispatch) => {
  ...
  try {
    const res = await axios.put(
      `http://localhost:4002/api/products/updateproduct/${productId}`,
      formData,
      config
    );
    dispatch(getAllProducts());
...
export const deleteProduct = (productId) => async (dispatch) => {
  ...
  try {
    await axios.delete(
      `http://localhost:4002/api/products/deleteproduct/${productId}`,
      config
    );
    dispatch(getAllProducts());
  } catch (error) {
...
export const getOneProduct = (productId) => async (dispatch) => {
  ...
  try {
    const res = await axios.get(
      `http://localhost:4002/api/products/getoneproduct/${productId}`,
      config
    );
    console.log(res.data);
    dispatch({
